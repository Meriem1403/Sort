...
import { Eye, EyeOff, User, Mail, MapPin, Calendar, ArrowRight, Check, Users, Glasses, type LucideIcon } from "lucide-react";
...
const GENERATIONS: { value: Generation; label: string; desc: string; icon: LucideIcon }[] = [
  { value: "jeune",  label: "Jeune",  desc: "18 – 35 ans", icon: User },
  { value: "parent", label: "Parent", desc: "35 – 60 ans", icon: Users },
  { value: "senior", label: "Senior", desc: "60 ans +",    icon: Glasses },
];
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {GENERATIONS.map(({ value, label, desc, icon: Icon }) => (
            <button key={value} type="button" onClick={() => setGeneration(value)}
              className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all text-center ${
                generation === value
                  ? "border-[#FF6B35] bg-[#FF6B35]/5 text-[#FF6B35]"
                  : "border-[#0D1B2A]/10 text-[#0D1B2A]/60 hover:border-[#0D1B2A]/25"
              }`}>
              <Icon size={22} strokeWidth={1.75} className={generation === value ? "text-[#FF6B35]" : "text-[#0D1B2A]/45"} />
              <span className="text-xs font-semibold" style={sans}>{label}</span>
              <span className="text-[10px]" style={mono}>{desc}</span>
            </button>
          ))}
        </div>
      </div>
